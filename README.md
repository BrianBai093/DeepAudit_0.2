...
│   │   │   ├── env_repair_agent.py     # Claude Code native env 修复
│   │   │   ├── code_compat_agent.py    # Claude Code 代码兼容修复
│   │   │   ├── claude_code_session.py  # Phase 2 Claude Code SDK session helper
...
3. `EnvRepairAgent.run()` / `CodeCompatAgent.run()`（仅 native conda/mamba env 失败或 `--phase2_force_env_repair`）  
   两者优先使用 Claude Code Agent SDK session。`EnvRepairAgent` 负责分析失败的原生 yml、执行 bounded 环境修复并写 `execution/env_repair/*`；`CodeCompatAgent` 负责在修复后的环境里做 import-only validation，必要时直接修改目标 repo 的最小兼容 patch，并写 `execution/code_compat/*`。如果 `claude-agent-sdk` 不可用，会退回到宿主侧的 bounded fallback。
...
4. `ExecutorAgent.run()`  
   ...
   训练产物和运行证据以 `execution/executor_outputs/phase2_execution_package.json` 为 Phase 3 的规范输入；executor 同时写 `run_manifest.json`、`executor_results.json`、session logs 和 per-experiment logs。

5. `Phase2Orchestrator`  
...
| `P2C_DISABLE_CLAUDE_REPAIR_SDK` | 禁用 EnvRepair/CodeCompat 的 Claude Code SDK 主路径，使用宿主 fallback | `0` |
| `P2C_ENV_REPAIR_SDK_TIMEOUT_SEC` | EnvRepairAgent Claude Code session 超时 | `2400` |
| `P2C_CODE_COMPAT_SDK_TIMEOUT_SEC` | CodeCompatAgent Claude Code session 超时 | `900` |
