...
def test_executor_prompt_moves_outputs_outside_repo_when_artifacts_are_inside_repo(tmp_path: Path, monkeypatch) -> None:
    repo_dir = tmp_path / "repo"
    repo_dir.mkdir()
    (repo_dir / "README.md").write_text("python train.py\n", encoding="utf-8")
    (repo_dir / "train.py").write_text("print('ok')\n", encoding="utf-8")

    artifacts = ArtifactManager(repo_dir / "artifacts", "run_inside_repo")
    artifacts.ensure_tree()
    artifacts.write_json(
        "fingerprint/claims_ir.json",
        {
    ...
    def fake_session(env_mgr, prompt, cwd, timeout_sec=600):
        seen["prompt"] = prompt
        output_line = next(line for line in prompt.splitlines() if line.startswith("External output directory"))
        external_outputs = Path(output_line.split(":", 1)[1].strip())
        seen["outputs_dir"] = str(external_outputs)
        external_outputs.mkdir(parents=True, exist_ok=True)
        stdout_log = external_outputs / "experiment_exp_01_stdout.log"
        stderr_log = external_outputs / "experiment_exp_01_stderr.log"
        narrative_log = external_outputs / "experiment_exp_01_narrative.log"
        stdout_log.write_text("METRIC:accuracy=0.91\n", encoding="utf-8")
        stderr_log.write_text("", encoding="utf-8")
        narrative_log.write_text("completed\n", encoding="utf-8")
        (external_outputs / "executor_results.json").write_text(
            json.dumps(
                {
                    "runs": [
                        {
                            ...
                            "logs": {
                                ...
                                "narrative": str(narrative_log),
                                "activity": str(external_outputs / "executor_activity.jsonl"),
                            },
                            "reason_codes": [],
                        }
                    ]
    ...
    assert result["success"] is True
    assert not Path(seen["outputs_dir"]).resolve().is_relative_to(repo_dir.resolve())
    assert str(artifacts.path("execution/executor_outputs").resolve()) not in seen["prompt"]
